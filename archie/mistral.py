...
class InputEmbeddings(nn.Module):
    ...
    def forward(self, x):
        return self.embedding(x) * math.sqrt(
            self.d_model
        )  # Multiply by sqrt(d_model) to increase variance which tends to decrease
...
class RotaryPostionalEncoding(nn.Module):
    ...
    @staticmethod
    def compute_theta_params(
        d_model: int, seq_len: int, device: str, theta: float = 10000.0
    ):
        ...
        freqs_cisp = torch.polar(
            torch.ones_like(freqs), freqs
        )  # we don't care about the scalars for now, we set them to 1
        return freqs_cisp

    @staticmethod
    def apply_rotary_embeddings(
        x: torch.Tensor, freq_complex: torch.Tensor, device: str
    ):
...
class GQA(nn.Module):
    ...
    def forward(self, x: torch.Tensor, start_pos: int, freq_complex: torch.Tensor):
        ...
        xq = RotaryPostionalEncoding(self.args).apply_rotary_embeddings(
            xq, freq_complex, device=x.device
        )
        xk = RotaryPostionalEncoding(self.args).apply_rotary_embeddings(
            xk, freq_complex, device=x.device
        )
        ...
        # Apply unrotation
        keys = torch.cat(
            [GQA(self.args).unrotate(keys[i, :, :, :] for i in range(keys.shape[0]))],
            dim=0,
        )
        values = torch.cat(
            [
                GQA(self.args).unrotate(
                    values[i, :, :, :] for i in range(values.shape[0])
                )
            ],
            dim=0,
        )
        ...
        keys = keys.transpose(
            1, 2
        )  # (batch_size, n_local_heads, cache_len + seqlen, head_dim)
